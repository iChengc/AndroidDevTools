...
:InstallStart
echo 请输入安装包的名字，若要安装最近更改的安装包猛敲回车键.
set packageWillInstall=""
set /p packageWillInstall=

if %packageWillInstall%=="" goto GetDefaultPackage
goto DoInstall

rem get the latest modified apk file name.
:GetDefaultPackage
rem echo %currentDir%
for /f "delims=" %%i in ('dir *.apk /b/a-d/od') do (
	set file_array=%%i
)
set packageWillInstall="%file_array%"
